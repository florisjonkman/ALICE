...
enum {kExpo=0, kLinear, kPol2, kThrExpo,kPowEx, kNoBackg};
...
void SetFitParameters(){
  ...
  // Fit?             Rebin             BackgroudFunc        SignalFunct           MinimumFitRange         MaxFitRange          // if > 0 : fixed, if = 0 : not fixed
  isFit[0] = kTRUE;  rebin[0]  = 10;   typeB[0]  = kPol2;   typeS[0]  = kGaus;    minFit[0]  = 2.150;    maxFit[0]  = 2.440;   fixSigma[0] = sigmaMC[0];  //0.008
  isFit[1] = kTRUE;  rebin[1]  = 10;   typeB[1]  = kPol2;   typeS[1]  = kGaus;    minFit[1]  = 2.150;    maxFit[1]  = 2.440;   fixSigma[1] = sigmaMC[1];  //0.008
  isFit[2] = kTRUE;  rebin[2]  = 10;   typeB[2]  = kPol2;   typeS[2]  = kGaus;    minFit[2]  = 2.150;    maxFit[2]  = 2.440;   fixSigma[2] = sigmaMC[2]; //0.009
  isFit[3] = kTRUE;  rebin[3]  = 12;   typeB[3]  = kPol2;   typeS[3]  = kGaus;    minFit[3]  = 2.150;    maxFit[3]  = 2.440;   fixSigma[3] = sigmaMC[3]; //0.011
  isFit[4] = kTRUE;  rebin[4]  = 16;   typeB[4]  = kPol2;   typeS[4]  = kGaus;    minFit[4]  = 2.150;    maxFit[4]  = 2.430;   fixSigma[4] = sigmaMC[4]; //0.012
  isFit[5] = kFALSE;  rebin[5]  = 40;   typeB[5]  = kPol2;   typeS[5]  = kGaus;    minFit[5]  = 2.090;    maxFit[5]  = 2.510;   fixSigma[5] = sigmaMC[5]; //0.016
  // isFit[6] = kTRUE;   rebin[6]  = 2;   typeB[6]  = kNoBackg;   typeS[6]  = kGaus;    minFit[6]  = 2.150;   maxFit[6]  = 2.440;
  // isFit[7] = kTRUE;    rebin[7]  = 2;   typeB[7]  = kNoBackg;   typeS[7]  = kGaus;    minFit[7]  = 2.150;   maxFit[7]  = 2.440;
  
  // Saving fitparameters in file
  FILE *fp = fopen(Form("%s/SetParameters.csv",gOutputDirname.Data()), "w");
  fprintf(fp, "pTBin, pTLow, pTUp, isFit, Rebin, BgFunc, SigFunc, MinRange, MaxRange,FixSigma\n");
  for(Int_t iPtBin = 0; iPtBin < nPtBins; iPtBin++){
    // pT Bin Ranges
    Double_t pTBinLow = ptBins[iPtBin];
    Double_t pTBinUp  = ptBins[iPtBin+1];
    fprintf(fp,"%d, %.1f, %.1f, %d, %d, %d, %d, %.3f, %.3f, %.3f \n",iPtBin+1,pTBinLow,pTBinUp,isFit[iPtBin],rebin[iPtBin],typeB[iPtBin],typeS[iPtBin],minFit[iPtBin],maxFit[iPtBin],fixSigma[iPtBin]);
  }
  fclose(fp);
...
void MassFitter(TFile* outputFile, Bool_t flagDraw = kTRUE
  ...
  if(flagDraw){
    ...
    cPulls = new TCanvas("cPulls", "Pulls", 1200, 550);
    cPulls->Divide(3,2);
  }
  ...
  Double_t nFit=0;
    ...
    Double_t minMassForPlot = minFit[iPtBin];
    Double_t maxMassForPlot = maxFit[iPtBin];
    ...
    // This implementation was added as GetOverBackgroundResidualsAndPulls gave an error if there was no error assigned
    Printf("\n==> Checking if no bin has no errors, if so assign 1 (needed for GetOverBackgroundResidualsAndPulls)");
    for (Int_t i = 1; i <= nBinsRebinned; ++i)
    {
        if(hRebinned[iPtBin]->GetBinError(i) == 0){
          Printf("Bin = %d, Center = %.4f, Content = %.4f ± %.4f",i,hRebinned[iPtBin]->GetBinCenter(i),hRebinned[iPtBin]->GetBinContent(i),hRebinned[iPtBin]->GetBinError(i));
          hRebinned[iPtBin]->SetBinError(i,1);
          Printf("> Bin error set to 1");
        }
    }
    Printf("");
    ...
    // Fit the function
  	Bool_t out=fitter[iPtBin]->MassFitter(0);
  	if(!out){
    ...
    fitter[iPtBin]                         ->Signal(nSigmaCount,dSignal,dSignalErr);
    fitter[iPtBin]                         ->Background(nSigmaCount,dBackground,dBackgroundErr);
    fitter[iPtBin]                         ->Significance(nSigmaCount,dSignificance,dSignificanceErr);
    ...
    if(flagDraw){
        ...
        if(dChiSquare[iPtBin]>0 && dChiSquare[iPtBin]<1000){
          ...
          if(typeB[iPtBin]!=kNoBackg) paveSigBG->AddText(Form("S/B (%d#sigma) %.4f",nSigmaCount,dSignal/dBackground));
    ...
    cPulls->cd();
    // Pulls
    if(flagDraw) cPulls->cd(iPtBin+1);
    TH1F *hPulls         = (TH1F*)hRebinned[iPtBin]->Clone(Form("hPulls_pT_%.0f_%.0f",pTBinLow,pTBinUp));
    TH1F *hResidualTrend = (TH1F*)hRebinned[iPtBin]->Clone(Form("hResidualTrend_pT_%.0f_%.0f",pTBinLow,pTBinUp));  
    TH1F *hPullsTrend    = (TH1F*)hRebinned[iPtBin]->Clone(Form("hPullsTrend_pT_%.0f_%.0f",pTBinLow,pTBinUp));
    TH1F *hsigsub = fitter[iPtBin]->GetOverBackgroundResidualsAndPulls(hMin,hMax,hPulls,hResidualTrend,hPullsTrend); 
    hsigsub->SetName(Form("hSigSub_pT_%.0f_%.0f",pTBinLow,pTBinUp));

    Double_t residualMinimum = 10e10;
    Double_t residualsMaximum = -10e10;
    if(hResidualTrend){
      for (Int_t i = 1; i <= hResidualTrend->GetNbinsX(); ++i)
      {
        if(hResidualTrend->GetBinCenter(i) >= hMin &&  hResidualTrend->GetBinCenter(i) <= hMax){
          if(hResidualTrend->GetBinContent(i) < residualMinimum)  residualMinimum = hResidualTrend->GetBinContent(i);
          if(hResidualTrend->GetBinContent(i) > residualsMaximum) residualsMaximum = hResidualTrend->GetBinContent(i);
        }
      }
      Printf("Min residuals = %f",residualMinimum);
      Printf("Max residuals = %f",residualsMaximum);
      hResidualTrend->GetYaxis()->SetRangeUser(residualMinimum-0.25*(residualsMaximum-residualMinimum),residualsMaximum+0.25*(residualsMaximum-residualMinimum));
      if(flagDraw) hResidualTrend->Draw(); 
    }
    ...
    hSigmaMC->SetBinContent(iPtBin+1,sigmaMC[iPtBin]);
    hSigmaMC->SetBinError(iPtBin+1,sigmaMCErr[iPtBin]);
    hMeanMC->SetBinContent(iPtBin+1,meanMC[iPtBin]);
    hMeanMC->SetBinError(iPtBin+1,meanMCErr[iPtBin]);
  ...
  TLegend* legMassSigma=new TLegend(0.575256,0.132013,0.891184,0.235121);
  ...
  hMassMean->Draw("PE");
  legMassSigma->AddEntry(hMassMean,"From fit","PL");
  hMeanMC->GetXaxis()->SetTitle("#it{p}_{T} (GeV/c)");
  hMeanMC->GetYaxis()->SetTitle("Mean (GeV/c^{2})");
  hMeanMC->GetYaxis()->SetTitleOffset(1.6);
  hMeanMC->SetTitle("Mean from Monte Carlo");
  hMeanMC->SetMarkerStyle(25);
  hMeanMC->SetMarkerColor(kRed);
  hMeanMC->SetLineColor(kRed);
  hMeanMC->SetLineStyle(2);
  hMeanMC->Draw("PESAME");
  legMassSigma->AddEntry(hMeanMC,"From Monte Carlo","PL");
  ...
  hSigmaMC->GetYaxis()->SetTitleOffset(1.6);
  hSigmaMC->SetTitle("Sigma from Monte Carlo");
  hSigmaMC->SetMarkerStyle(25);
  hSigmaMC->SetMarkerColor(kRed);
  hSigmaMC->SetLineColor(kRed);
  hSigmaMC->SetLineStyle(2);
  hSigmaMC->Draw("PESAME");
...
// Plot of Yield
  ...
  cPulls        ->SaveAs(Form("%s/Pulls_Pt.pdf"          ,gOutputDirname.Data()));
...
void HFMassFitter(TString fileName = "InvariantMassOptimalBDT_mult_1_100.root",
